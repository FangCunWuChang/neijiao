...
void LogOffset(CString log, CString strDate = _T(""))
{
	Log mainLog;
	mainLog.OffsetLogInit();
	if (strDate == "")
	{
		mainLog << mainLog.GetTimeStr() << log.GetBuffer(0) << endl;
	}
	...
	{
		mainLog << strDate.GetBuffer(0) << log.GetBuffer(0) << endl;
		strDate.ReleaseBuffer();
	}
	//mainLog << mainLog.GetTimeStr()<< log.GetBuffer(128)<< endl ;
	log.ReleaseBuffer();
	//CSingleton* pSng = CSingleton::GetInstance();
	//CString strS = mainLog.GetTimeStr().c_str();
	//pSng->_csInfo.Lock();
	//pSng->_Info.push_back(strS + log);
	//pSng->_csInfo.Unlock();
}
